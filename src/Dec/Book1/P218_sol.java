...
public class P218_sol {
    ...
    private static boolean match(String pattern, String fileName) {

        int pos = 0;

        while (pos < fileName.length() && pos < pattern.length() &&
                (pattern.charAt(pos) == '?' || pattern.charAt(pos) == fileName.charAt(pos)))
            pos += 1;
        ...
        if (pattern.charAt(pos) == '*')
            for (int skip = 0; pos + skip <= fileName.length(); skip++)
                if (match(pattern.substring(pos + 1), fileName.substring(pos + skip)))
                    return true;
