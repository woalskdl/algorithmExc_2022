...
public class P201_sol {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < c; i++) {
            ...
            int[] fans = new int[fansCnt];

            for (int j = 0; j < membersCnt; j++)
                members[j] = membersLine.charAt(j) == 'F' ? 0 : 1;

            for (int j = 0; j < fansCnt; j++)
                fans[j] = fansLine.charAt(j) == 'F' ? 0 : 1;

            int[] mul = karatsuba(members, fans);
            int result = 0;
    ...
    private static int[] karatsuba(int[] members, int[] fans) {
        int[] result = new int[members.length + fans.length - 1];

        for (int i = 0; i < result.length; i++) {
            int num = 0;
            for (int j = Math.min(i, fans.length - 1), k = 0; j > 0 && k < members.length; j--, k++)
                num += members[k] * fans[j];
